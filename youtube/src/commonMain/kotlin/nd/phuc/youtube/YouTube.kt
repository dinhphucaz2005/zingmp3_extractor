...
import nd.phuc.youtube.models.AccountInfo
import nd.phuc.youtube.models.AlbumItem
import nd.phuc.youtube.models.Artist
import nd.phuc.youtube.models.ArtistItem
import nd.phuc.youtube.models.BrowseEndpoint
import nd.phuc.youtube.models.GridRenderer
import nd.phuc.youtube.models.MusicCarouselShelfRenderer
import nd.phuc.youtube.models.PlaylistItem
import nd.phuc.youtube.models.SearchSuggestions
import nd.phuc.youtube.models.SongItem
import nd.phuc.youtube.models.WatchEndpoint
import nd.phuc.youtube.models.WatchEndpoint.WatchEndpointMusicSupportedConfigs.WatchEndpointMusicConfig.Companion.MUSIC_VIDEO_TYPE_ATV
import nd.phuc.youtube.models.YouTubeClient.Companion.ANDROID_MUSIC
import nd.phuc.youtube.models.YouTubeClient.Companion.IOS
import nd.phuc.youtube.models.YouTubeClient.Companion.TVHTML5
import nd.phuc.youtube.models.YouTubeClient.Companion.WEB_REMIX
import nd.phuc.youtube.models.YouTubeLocale
import nd.phuc.youtube.models.getContinuation
import nd.phuc.youtube.models.oddElements
import nd.phuc.youtube.models.response.AccountMenuResponse
import nd.phuc.youtube.models.response.BrowseResponse
import nd.phuc.youtube.models.response.GetQueueResponse
import nd.phuc.youtube.models.response.GetSearchSuggestionsResponse
import nd.phuc.youtube.models.response.NextResponse
import nd.phuc.youtube.models.response.PipedResponse
import nd.phuc.youtube.models.response.PlayerResponse
import nd.phuc.youtube.models.response.SearchResponse
import nd.phuc.youtube.pages.AlbumPage
import nd.phuc.youtube.pages.ArtistItemsContinuationPage
import nd.phuc.youtube.pages.ArtistItemsPage
import nd.phuc.youtube.pages.ArtistPage
import nd.phuc.youtube.pages.BrowseResult
import nd.phuc.youtube.pages.ExplorePage
import nd.phuc.youtube.pages.HomePage
import nd.phuc.youtube.pages.MoodAndGenres
import nd.phuc.youtube.pages.NewReleaseAlbumPage
import nd.phuc.youtube.pages.NextPage
import nd.phuc.youtube.pages.NextResult
import nd.phuc.youtube.pages.PlaylistContinuationPage
import nd.phuc.youtube.pages.PlaylistPage
import nd.phuc.youtube.pages.RelatedPage
import nd.phuc.youtube.pages.SearchPage
import nd.phuc.youtube.pages.SearchResult
import nd.phuc.youtube.pages.SearchSuggestionPage
import nd.phuc.youtube.pages.SearchSummary
import nd.phuc.youtube.pages.SearchSummaryPage
...
import kotlinx.serialization.json.jsonPrimitive
import nd.phuc.youtube.models.YouTubeClient.Companion.WEB
import nd.phuc.youtube.models.response.GetTranscriptResponse

/**
 * Parse useful data with [YouTubeMusicApi] sending requests.
...
object YouTube {
    ...
    var locale: YouTubeLocale
        get() = youTubeMusicApi.locale
        set(value) {
            youTubeMusicApi.locale = value
        }
    var visitorData: String
        get() = youTubeMusicApi.visitorData
        set(value) {
            youTubeMusicApi.visitorData = value
        }
    var cookie: String?
        get() = youTubeMusicApi.cookie
        set(value) {
            youTubeMusicApi.cookie = value
        }
    var useLoginForBrowse: Boolean
        get() = youTubeMusicApi.useLoginForBrowse
        set(value) {
            youTubeMusicApi.useLoginForBrowse = value
        }

    suspend fun searchSuggestions(query: String): Result<SearchSuggestions> = runCatching {
        val response =
            youTubeMusicApi.getSearchSuggestions(WEB_REMIX, query).body<GetSearchSuggestionsResponse>()
        SearchSuggestions(
    ...
    suspend fun searchSummary(query: String): Result<SearchSummaryPage> = runCatching {
        val response = youTubeMusicApi.search(WEB_REMIX, query).body<SearchResponse>()
        SearchSummaryPage(
    ...
    suspend fun search(query: String, filter: SearchFilter): Result<SearchResult> = runCatching {
        val response = youTubeMusicApi.search(WEB_REMIX, query, filter.value).body<SearchResponse>()
        SearchResult(
    ...
    suspend fun searchContinuation(continuation: String): Result<SearchResult> = runCatching {
        val response =
            youTubeMusicApi.search(WEB_REMIX, continuation = continuation).body<SearchResponse>()
        SearchResult(
    ...
    suspend fun album(browseId: String, withSongs: Boolean = true): Result<AlbumPage> =
        runCatching {
            val response = youTubeMusicApi.browse(WEB_REMIX, browseId).body<BrowseResponse>()
            val playlistId =
    ...
    suspend fun albumSongs(playlistId: String): Result<List<SongItem>> = runCatching {
        var response = youTubeMusicApi.browse(WEB_REMIX, "VL$playlistId").body<BrowseResponse>()
        ...
        while (continuation != null) {
            response = youTubeMusicApi.browse(
                client = WEB_REMIX,
    ...
    suspend fun artistItems(endpoint: BrowseEndpoint): Result<ArtistItemsPage> = runCatching {
        val response =
            youTubeMusicApi.browse(WEB_REMIX, endpoint.browseId, endpoint.params).body<BrowseResponse>()
    ...
    suspend fun artistItemsContinuation(continuation: String): Result<ArtistItemsContinuationPage> =
        runCatching {
            val response =
                youTubeMusicApi.browse(WEB_REMIX, continuation = continuation).body<BrowseResponse>()
    ...
    suspend fun playlist(playlistId: String): Result<PlaylistPage> = runCatching {
        val response = youTubeMusicApi.browse(
            client = WEB_REMIX,
    ...
    suspend fun home(): Result<HomePage> = runCatching {
        var response = youTubeMusicApi.browse(WEB_REMIX, browseId = "FEmusic_home").body<BrowseResponse>()
    ...
    suspend fun explore(): Result<ExplorePage> = runCatching {
        val response =
            youTubeMusicApi.browse(WEB_REMIX, browseId = "FEmusic_explore").body<BrowseResponse>()
        ExplorePage(
    ...
    suspend fun newReleaseAlbums(): Result<List<AlbumItem>> = runCatching {
        val response = youTubeMusicApi.browse(WEB_REMIX, browseId = "FEmusic_new_releases_albums")
            .body<BrowseResponse>()
    ...
    suspend fun moodAndGenres(): Result<List<MoodAndGenres>> = runCatching {
        val response = youTubeMusicApi.browse(WEB_REMIX, browseId = "FEmusic_moods_and_genres")
            .body<BrowseResponse>()
    ...
    suspend fun browse(browseId: String, params: String?): Result<BrowseResult> = runCatching {
        val response =
            youTubeMusicApi.browse(WEB_REMIX, browseId = browseId, params = params).body<BrowseResponse>()
        BrowseResult(
            title = response.header?.musicHeaderRenderer?.title?.runs?.firstOrNull()?.text,
    ...
    suspend fun player(videoId: String, playlistId: String? = null): Result<PlayerResponse> =
        runCatching {
            ...
            if (this.cookie != null) { // if logged in: try ANDROID_MUSIC client first because IOS client does not play age restricted songs
                playerResponse =
                    youTubeMusicApi.player(ANDROID_MUSIC, videoId, playlistId).body<PlayerResponse>()
                if (playerResponse.playabilityStatus.status == "OK") {
                    return@runCatching playerResponse
                }
            }
            playerResponse = youTubeMusicApi.player(IOS, videoId, playlistId).body<PlayerResponse>()
            if (playerResponse.playabilityStatus.status == "OK") {
                return@runCatching playerResponse
            }
            val safePlayerResponse =
                youTubeMusicApi.player(TVHTML5, videoId, playlistId).body<PlayerResponse>()
            if (safePlayerResponse.playabilityStatus.status != "OK") {
                return@runCatching playerResponse
            }
            val audioStreams = youTubeMusicApi.pipedStreams(videoId).body<PipedResponse>().audioStreams
    ...
    suspend fun next(endpoint: WatchEndpoint, continuation: String? = null): Result<NextResult> =
        runCatching {
            val response = youTubeMusicApi.next(
                WEB_REMIX,
                endpoint.videoId,
                endpoint.playlistId,
    ...
    suspend fun related(endpoint: BrowseEndpoint): Result<RelatedPage> = runCatching {
        val response = youTubeMusicApi.browse(WEB_REMIX, endpoint.browseId).body<BrowseResponse>()
    ...
    suspend fun queue(
        ...
        youTubeMusicApi.getQueue(WEB_REMIX, videoIds, playlistId).body<GetQueueResponse>().queueDatas
            .mapNotNull {
    ...
    suspend fun transcript(videoId: String): Result<String> = runCatching {
        val response = youTubeMusicApi.getTranscript(WEB, videoId).body<GetTranscriptResponse>()
    ...
    suspend fun visitorData(): Result<String> = runCatching {
        Json.parseToJsonElement(youTubeMusicApi.getSwJsData().bodyAsText().substring(5))
            .jsonArray[0]
            .jsonArray[2]
    ...
    suspend fun accountInfo(): Result<AccountInfo> = runCatching {
        youTubeMusicApi.accountMenu(WEB_REMIX).body<AccountMenuResponse>()
...
private fun String.format(lng: Long, lng2: Long, lng3: Long): CharSequence {
